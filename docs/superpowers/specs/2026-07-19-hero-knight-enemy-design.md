...
There is no pixel-art enemy sprite matching Hero Knight's visual style anywhere in the project. Rather than block this design on new art, the enemy uses a generated placeholder capsule sprite (same approach `HeroKnightSandboxSetup.cs`'s `BuildScene()` already uses for `GroundSprite.png`), with SpriteRenderer color tints standing in for state feedback (no Animator, no sprite animation). Replacing this with real art is future work.
...
1. **Enemy system** — new folder `Assets/Scripts/HeroKnight/Enemy/`, namespace `HeroKnightSandbox.Enemy`. Not integrated with `Simulation`, `Platformer.Mechanics.EnemyController`, or `Platformer.Mechanics.Health`. The only piece of the existing template reused is `Platformer.Mechanics.PatrolPath` (a plain position-oscillator `MonoBehaviour` with zero `Simulation` coupling — safe to reuse without violating the self-contained architecture).
...
- `Platformer.Mechanics.PatrolPath PatrolPath`
...
- `int AttackDamage = 1`
- `float AttackWindup = 0.3f` — seconds from entering Attack to the damage-application instant.
- `float AttackCooldown = 0.5f` — seconds after the damage instant before returning to Patrol.
...
namespace HeroKnightSandbox.Enemy
{
    ...
    [RequireComponent(typeof(Platformer.Mechanics.PatrolPath))]
    public class EnemyController : MonoBehaviour
    {
        ...
        private void Awake()
        {
            context = new EnemyContext
            {
                ...
                PatrolPath = GetComponent<Platformer.Mechanics.PatrolPath>(),
                Player = player,
            };
...
`Assets/Scripts/HeroKnight/Enemy/States/PatrolState.cs`. On `Enter()`, creates a `PatrolPath.Mover` via `Context.PatrolPath.CreateMover(Context.MoveSpeed)` and moves `Context.Transform.position` toward `mover.Position` each `Tick()` (same read-only oscillator pattern the template's own `EnemyController.Update()` uses, adapted to directly set position rather than feed a separate move-axis field, since this enemy has no physics-driven `AnimationController`). Each `Tick()`, computes `Vector2.Distance(Context.Transform.position, Context.Player.transform.position)`; if less than `Context.AttackRange`, calls `Controller.ChangeState(Controller.Attack)`.

### `AttackState`

`Assets/Scripts/HeroKnight/Enemy/States/AttackState.cs`. On `Enter()`: stops patrol movement, resets an internal timer to 0, sets `Context.SpriteRenderer.color` to a distinct attack-tell color (red). Each `Tick()`, increments the timer by `Time.deltaTime`:
- When the timer crosses `Context.AttackWindup` (once, guarded by a bool so it only fires once per attack): if the player is still within `Context.AttackRange`, call `Context.Player.TakeDamage(Context.AttackDamage)`.
- When the timer crosses `Context.AttackWindup + Context.AttackCooldown`: reset `Context.SpriteRenderer.color` to white and `Controller.ChangeState(Controller.Patrol)`.

### `HurtState`

`Assets/Scripts/HeroKnight/Enemy/States/HurtState.cs`. On `Enter()`: sets `Context.SpriteRenderer.color` to a flinch color (white flash / brighter tint), resets a timer to 0. Each `Tick()`: once the timer exceeds `Context.HurtDuration`, restores the sprite color and calls `Controller.ChangeState(Controller.Patrol)`.

### `DeadState`

`Assets/Scripts/HeroKnight/Enemy/States/DeadState.cs`. On `Enter()`: disables `Context.PatrolPath` movement (no further `Tick()`-driven position changes), resets a timer to 0. Each `Tick()`: linearly fades `Context.SpriteRenderer.color.a` from 1 to 0 over `Context.DeathFadeDuration`; once it reaches 0, calls `Object.Destroy(Controller.gameObject)`.
...
This is the enemy's only entry point into the player's state machine — `EnemyState.AttackState` calls `Context.Player.TakeDamage(Context.AttackDamage)` directly, no events, no `Simulation` scheduling, consistent with both systems being self-contained. No death handling: `CurrentHP` can go to or below 0 with no special-cased behavior beyond the normal Hurt flinch — a future spec adds death/respawn.
...
- Generates the placeholder enemy sprite the same way `BuildScene()` already generates `GroundSprite.png` — a small solid-color capsule/rounded-rect texture, saved to `Assets/Prefabs/EnemySprite.png`, imported with `SpriteMeshType.FullRect`.
- Creates exactly 2 enemy `GameObject`s directly in `HeroKnightSandbox.unity` (not as a reusable prefab, matching this scene's existing pattern of building most things inline via the automation script) — enough to test both an isolated encounter and, later, behavior when two enemies are attackable in the same area, without adding real variety. Each has: `SpriteRenderer` (using the generated sprite), `CapsuleCollider2D` sized to the placeholder sprite, `Platformer.Mechanics.PatrolPath` (with a short `startPosition`/`endPosition` range on flat ground near the player's spawn — reachable without requiring wall-slide/ledge-grab to reach), and `HeroKnightSandbox.Enemy.EnemyController` with its `player` field wired to the scene's existing `HeroKnight` instance (found via the same `GameObject.Find`/hierarchy-lookup pattern `BuildScene()` already uses elsewhere in this file).
- Idempotent the same way the rest of the script is: since `BuildScene()` already rebuilds the whole scene with `NewSceneMode.Single`, re-running `4 Build Scene` naturally wipes and recreates enemies too — `5 Build Enemies` only needs to guard against being run twice in a row *without* an intervening scene rebuild (check for an existing `GameObject` by name before creating, matching the existing convention in this file for other idempotent steps).

## Testing plan

Manual Unity Editor Play-mode pass, same style as the movement controller's Task 12, covering:

- Enemy patrols correctly between its `PatrolPath` endpoints without player nearby.
- Approaching within `AttackRange` stops patrol and triggers the enemy's Attack state (visible via the red color tell); moving back out of range before the windup completes should still let the attack resolve as designed (the range check only happens once, at windup) — confirm this matches intent or tune `AttackWindup` if it feels wrong.
- Player's attack combo damages the enemy: confirm `CurrentHP` depletes per hit and it dies (fades out, `GameObject` destroyed) after `MaxHP` hits.
